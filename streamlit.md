# RELIANCE BiLSTM — Streamlit Local Deployment

> **Goal**: Run the trained model as a Streamlit web app on
> `http://localhost:8501` with one command.
...
Flask                    Streamlit
─────────────────────    ─────────────────────────────
Write HTML manually      No HTML needed
Write API routes         No routes needed
Separate frontend        Python only — UI + logic together
More setup               pip install + one file = done
```
...
reliance_streamlit/
│
├── app.py                 ← entire app (UI + prediction logic)
│
├── model/
...
## Step 2 — Copy Your Artefacts

```bash
mkdir -p reliance_streamlit/model
mkdir -p reliance_streamlit/data

cp RELIANCE_bilstm_model.keras   reliance_streamlit/model/
cp RELIANCE_scaler.pkl           reliance_streamlit/model/
cp RELIANCE_target_scaler.pkl    reliance_streamlit/model/
cp RELIANCE_features.csv         reliance_streamlit/model/
cp RELIANCE.csv                  reliance_streamlit/data/
```
...
@st.cache_resource
def load_artefacts():
    model         = load_model(f"{MODEL_DIR}/RELIANCE_bilstm_model.keras")
    scaler        = joblib.load(f"{MODEL_DIR}/RELIANCE_scaler.pkl")
    target_scaler = joblib.load(f"{MODEL_DIR}/RELIANCE_target_scaler.pkl")
    features      = (pd.read_csv(f"{MODEL_DIR}/RELIANCE_features.csv")
                       ["feature"].tolist())
...
# ── Header ───────────────────────────────────────────────────
st.title("📈 RELIANCE — BiLSTM Stock Forecast")
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Model: RELIANCE_BiLSTM")
...
```
1. Page loads → model artefacts load automatically (cached)
2. Click  "Run Forecast"  button
3. App runs the feature pipeline on RELIANCE.csv
4. Displays:
      ✓ 4 metric cards  (last price, day 1, day N, up/down count)
      ✓ Forecast table  (colour-coded green/red)
      ✓ Price chart     (history + forecast ribbon)
      ✓ Returns chart   (daily % bar chart)
...
| Code block | Purpose |
|---|---|
| `@st.cache_resource` | Loads model once — not reloaded on every button click |
| `@st.cache_data` | Caches the CSV + feature pipeline output |
| `st.slider` | Controls how many forecast days to display |
| `st.metric` | The 4 summary cards at the top |
| `st.dataframe(styled)` | Colour-coded forecast table |
| `st.pyplot` | Matplotlib price + returns charts |
| `st.download_button` | Saves forecast as CSV to your computer |

---

## Every Time Your System Restarts
...
That is all. The model loads from disk in seconds. No retraining.
...
| Error | Fix |
|---|---|
| `File not found: model/RELIANCE_bilstm_model.keras` | Copy artefacts into `model/` folder |
| `ModuleNotFoundError: streamlit` | `pip install streamlit` |
| `Port 8501 already in use` | `streamlit run app.py --server.port 8502` |
| Charts not showing | `pip install matplotlib` |
| Slow first forecast | Normal — model warms up on first predict call |
...
```
[ ] reliance_complete_pipeline.py has been run (model trained)
[ ] Folder structure created
[ ] 4 model artefacts copied into model/
[ ] RELIANCE.csv copied into data/
[ ] pip install streamlit tensorflow scikit-learn pandas numpy joblib
[ ] streamlit run app.py
[ ] http://localhost:8501 opens in browser
[ ] "Run Forecast" shows the 10-row forecast table
```
